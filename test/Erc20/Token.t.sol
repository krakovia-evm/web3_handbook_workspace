...
import { RunScripts } from "../../scripts/Erc20/deploy.s.sol";
...
// ██║██║ ╚████║   ██║   ███████╗██║  ██║██║ ╚████║██║  ██║███████╗███████║
// ╚═╝╚═╝  ╚═══╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝╚══════╝╚══════╝
    function addLiquidity(uint tokensInLiq, uint ethtobuy) internal {
        token.approve(address(router), tokensInLiq);
        router.addLiquidityETH{value:ethtobuy}(
            address(token),
            tokensInLiq,
            tokensInLiq,
            ethtobuy,
...
// ╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝╚═╝        ╚═╝   ╚══════╝       ╚═╝   ╚══════╝╚══════╝   ╚═╝   
    ...
    function test_script_deploy() public {
        address deployerAddress = vm.envAddress("PKEY_ACC1_ADDRESS");
        // send ETH to deployer, otherwise it will fail
        vm.deal(deployerAddress, 10 ether);

        // run scripts
        RunScripts scripts = new RunScripts();
        scripts.run();
    }
