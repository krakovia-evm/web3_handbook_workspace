...
async function deploy_token_contract() {
    ...
    console.log("✅token online on: %s", token.address);
}
...
async function addLiquidity() {
    const amount = ethers.utils.parseUnits("100000", tokenDecimals)
    const amountETH = ethers.utils.parseEther("10")
    
    console.log("⚪approving router...")
    let tx = await token.approve(routerAddress, ethers.constants.MaxUint256)
    await tx.wait()
    console.log("Router approved.")

    console.log(`Ratio: ${amount/10**tokenDecimals} Token:${amountETH/10**18} ETH`)
    console.log("⚪Adding liquidity...")
    tx = await router.addLiquidityETH(token.address,
        amount, // minAmountToken
        amount, // AmountToken
        amountETH, // minAmountETH
